defmodule AboutWeb.ChatLive.Room do
  ...
  defp create_new_participant(socket, room_id, nickname) do
    ...
    participant = case existing_participants do
      [existing | _] ->
        # 기존 참여자 재사용
        existing
      [] ->
  ...
  def handle_info({:update_session, _key, value}, socket) do
    ...
    {:noreply, push_event(socket, "save_participant", %{room_id: socket.assigns.room.id, participant_id: value})}
  end
  ...
  def handle_info(:load_data, socket) do
    ...
    online_participants = ChatPresence.list_participants_with_meta(room_id)
  ...
  @impl true
  def handle_info({:participant_joined, %{participant: participant, session_id: session_id}}, socket) do
    system_message = %{
  ...
  @impl true
  def handle_info({:participant_left, %{participant_id: _participant_id, nickname: nickname, session_id: session_id}}, socket) do
    system_message = %{
      id: Ash.UUID.generate(),
  ...
  def handle_info({:presence_leave, session_id, meta}, socket) do
    ...
    if meta[:participant] do
      ...
      PubSub.broadcast(
        ...
        {:participant_left, %{participant_id: participant.id, nickname: meta.nickname, session_id: session_id}}
      )
    end
  ...
  def handle_event("leave_room", _, socket) do
    if socket.assigns.current_participant && socket.assigns.session_id do
      ...
      PubSub.broadcast(
        About.PubSub,
        "room:#{room_id}",
        {:participant_left, %{participant_id: participant_id, nickname: participant.nickname, session_id: session_id}}
      )
  ...
  def handle_event("change_nickname", %{"nickname" => new_nickname}, socket) do
    if socket.assigns.current_participant && socket.assigns.session_id && String.trim(new_nickname) != "" do
      participant = socket.assigns.current_participant
      room_id = socket.assigns.room.id
      session_id = socket.assigns.session_id
  ...
  @impl true
  def handle_event("restore_participant", %{"room_id" => room_id, "participant_id" => participant_id}, socket) do
    # 세션에서 기존 참여자 정보 복원
    case Ash.get(About.Chat.Participant, participant_id, domain: About.Chat) do
      {:ok, participant} ->
        session_id = socket.assigns.session_id
  ...
  def terminate(_reason, socket) do
    ...
    if socket.assigns[:current_participant] && socket.assigns[:room] && socket.assigns[:session_id] do
      participant = socket.assigns.current_participant
      room_id = socket.assigns.room.id
      session_id = socket.assigns.session_id
      ...
      if ChatPresence.participant_online?(room_id, session_id) do
        ...
        PubSub.broadcast(
          About.PubSub,
          "room:#{room_id}",
          {:participant_left, %{participant_id: participant.id, nickname: participant.nickname, session_id: session_id}}
        )
      end
    end
